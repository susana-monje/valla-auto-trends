...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // DATOS DE RESPALDO PARA QUE NO SALGAN CEROS
  ...
  if (loading) return <div className="p-10 text-center">Cargando VallaAuto Trends...</div>;
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {keywords.map((kw) => (
                ...
                >
                  <td className="p-4 font-semibold">{kw.name}</td>
                  <td className="p-4 text-center">{kw.volume}</td>
                  <td className="p-4 text-right font-bold text-green-600">{kw.trend}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-6">
          ...
          {selectedKeyword && (
            <div className="bg-blue-600 p-6 rounded-2xl shadow-lg text-white">
              <p className="text-sm opacity-80 uppercase font-bold">Potencial ROI</p>
              <p className="text-4xl font-black">{selectedKeyword.potential}%</p>
              <p className="mt-4 text-sm">Localización: {selectedKeyword.location}</p>
            </div>
          )}
        </div>
      </div>
